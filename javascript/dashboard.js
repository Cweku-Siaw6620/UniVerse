...
// Product Management
// Replace the existing product management section that uses localStorage with this:

// Product Management - using fresh store data from API
...
let currentStore = null; // Store the current store data
const planLimits = {
    'free': 15,
    'premium': 45,
    'organizational': 90
...
async function fetchStoreForProducts() {
    try {
        ...
        // Store ID for product fetching
        window.storeId = store._id;
        
        // Fetch products after we have the store
        await fetchProducts();
        ...
        console.error('Failed to get store for products:', error);
        ...
        try {
            ...
            console.error("Failed to get store from localStorage:", err);
        }
    }
}
...
function updateProductLimit() {
    productLimit.textContent = planLimits[currentPlan] || 15;
    updateProgressBar();
}

// Allow the plan-status fetch to update currentPlan
...
addProductForm.addEventListener('submit', async function(e) {
    ...
    // Check product limit
    if (products.length >= planLimits[currentPlan]) {
...
// Fetch products using the store ID
async function fetchProducts() {
    const storeId = currentStore?._id || window.storeId;
    if (!storeId) {
    ...
    recentProducts.forEach(product => {
        ...
        productElement.innerHTML = `
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="text-sm text-gray-400">GHC ${parseFloat(product.productPrice).toFixed(2)}</div>
...
// Initialize - fetch store data first, then products
async function initializeProductSection() {
    await fetchStoreForProducts();
    updateProductLimit();
}
