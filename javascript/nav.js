  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (user && user.picture) {
      ...
      // Create wrapper div for profile + dropdown
      ...
      dropdown.innerHTML = `
        <a href="/components/createStore.html" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Create your own Store</a>
      ...
      // Replace login link with dropdown profile
      if (loginLink) {
...
/*
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (user && user.picture) {
      ...
      let hasStore = false;
      try {
        const res = await fetch(`https://universe-api-uabt.onrender.com/api/store/exists?email=${encodeURIComponent(user.email)}`);
      ...
      // Dropdown menu HTML
      dropdown.innerHTML = `
        <a href="stores.html" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
